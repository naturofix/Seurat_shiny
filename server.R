...
shinyServer(function(input, output) {
  ...
  output$reduce_dataset_select_ui = renderUI({
    if(!is.null(input$select_dataset)){
      ...
      reduce_list = grep(input$select_dataset,rds_list,value = T)
        ...
        reduce_list_sub
        selectInput('reduce_select','Select sub datasets',reduce_list_sub,'')
       # }
    }
  })
  ...
  observeEvent(input$run_pca,{
    print('pca_1')
      if(input$pca_full_dataset == T){
        #if(!is.null(values$Seurat_data_scale)){
        #  data = values$Seurat_data_scale
        #}else{
          withProgress(message = 'readRDS',{
            data = readRDS(paste(dataset_path(),'_full'))
          })
        #}
      }else{
        if(!is.null(values$reduced_data)){
          data = values$reduced_data
        }else{
          withProgress(message = 'readRDS',{
            data = readRDS(paste(dataset_path(),'_reduced'))
          })
        }
      }
